...
```bash
# Auto-detect the project directory from current working directory
# Claude Code stores sessions under ~/.claude/projects/ with path separators replaced by dashes
PROJECT_DIR="$HOME/.claude/projects/$(pwd | sed 's|^/||; s|/|-|g')"
OUTPUT="/tmp/session-summary.jsonl"

if [ ! -d "$PROJECT_DIR" ]; then
  echo "ERROR: No session data found at $PROJECT_DIR"
  echo "Available projects:"
  ls "$HOME/.claude/projects/" | head -20
  exit 1
fi

# Calculate timestamp for 48 hours ago (in seconds since epoch)
CUTOFF_TIME=$(date -d '48 hours ago' +%s 2>/dev/null || date -v-48H +%s 2>/dev/null)

# Summarize: extract user requests, tool names, assistant text (truncated)
# Filter to last 48 hours only to keep summary focused and relevant
cat "$PROJECT_DIR"/*.jsonl 2>/dev/null | jq -c --arg cutoff "$CUTOFF_TIME" '
select(.type == "user" or .type == "assistant") |
select(
  (.timestamp | fromdateiso8601) >= ($cutoff | tonumber)
) |
{
  type,
  ts: .timestamp,
...
echo "Summary: $(wc -l < "$OUTPUT") messages, $(wc -c < "$OUTPUT" | xargs) bytes (last 48 hours)"
```
...
4. **Script automation** - Shell scripts to automate manual steps
...
1. **Re-reading the same file repeatedly** (7 times in one session)
   - Propose: Add key summaries to CLAUDE.md
...
3. **Manual repetitive shell commands**
   - Repeated the same commands each session
   - Propose: Create slash commands to automate them
